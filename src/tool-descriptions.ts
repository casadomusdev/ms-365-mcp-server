...
export const TOOL_DESCRIPTIONS: Record<string, string> = {
  ...
  'list-mail-folders': 'List existing folders in the signed-in mailbox. Use $select=id,displayName to resolve target folders (e.g., Archive via displayName="Archive" or "Inbox" for the inbox). IMPORTANT: When selecting a folder for list-mail-messages, use the folder\'s "id" field (NOT parentFolderId). Each folder object includes helper fields "folderIdToUse" and "_useThisIdForFolderQueries" that explicitly point to the correct ID to use. The "id" is the unique identifier for that specific folder. Folder creation is NOT supported by this MCP—reject such requests.',
